...
parser.add_argument('--model', type=str, default='mlp', choices=['mlp', 'simpleconv', 'resnet'],
                    help='Model')
...
def mmd_loss_cond(dim, z, label, z_shuffled, label_shuffled, sigmas):
    # loop over the different conditions (for example when conditioned on domain, evaluate for svhn and mnist)
    mmd_loss_sum = 0
    for m in range(dim):
        filter1 = (label==m)
...
# def restricted_sampling(z, f1_labels, f2_labels, dim, label):
#     z_shuffled = torch.tensor([]).to(use_device)
#     left_label_shuffled = torch.LongTensor([]).to(use_device)
#     right_label_shuffled = torch.LongTensor([]).to(use_device)

#     for c in range(dim):
#         # filter by bg/fg
#         my_filter = (label==c)
#         z_filtered = z[my_filter]

#         left_label_filtered = torch.clone(f1_labels)[my_filter]
#         right_label_filtered = torch.clone(f2_labels)[my_filter]

#         # shuffle inside of this bg/fg. This shuffles the first subspace.
#         perm = torch.randperm(len(z_filtered))
#         z_shuffled_tmp = torch.clone(z_filtered)
#         z_shuffled_tmp[:,:subspace_dim] = z_shuffled_tmp[perm,:subspace_dim]
#         z_shuffled = torch.cat([z_shuffled, z_shuffled_tmp])
#         left_label_shuffled = torch.cat([left_label_shuffled, left_label_filtered[perm]])
#         right_label_shuffled = torch.cat([right_label_shuffled, right_label_filtered[perm]])

#     return z_shuffled, left_label_shuffled, right_label_shuffled
    ...
    for images, labels in tqdm(train_loader):
        ...
        if args.mi_type == 'unconditional':
            ...
            for f1_value in list(set(f1_labels.detach().cpu().numpy())):  # Possible values for a1
                ...
                real_scores_f1 = f1_discriminator(torch.cat([cond_zs.float(), cond_left_labels.float()], dim=1))
                fake_scores_f1 = f1_discriminator(torch.cat([cond_zs_sub_shuffled.float(), cond_left_labels.float()], dim=1))
            ...
            for f2_value in list(set(f2_labels.detach().cpu().numpy())):
                ...
                real_scores_f2 = f2_discriminator(torch.cat([cond_zs.float(), cond_right_labels.float()], dim=1))
                fake_scores_f2 = f2_discriminator(torch.cat([cond_zs_sub_shuffled.float(), cond_right_labels.float()], dim=1))
