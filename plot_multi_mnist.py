...
fig, axs = plt.subplots(nrows=num_correlations, ncols=3, figsize=figsize,
...
for (i,mi_type) in enumerate(['cls', 'uncond', 'cond']):
  ...
  for (j,correlation) in enumerate(use_correlation_list):
    ...
    ax.plot(noise_levels_sorted, corr_accs,
            marker=marker, markersize=4, linewidth=2, color=color_train)
    ax.plot(noise_levels_sorted, anticorr_accs,
            marker=marker, markersize=4, linewidth=2, color=color_test_ac)
    ax.plot(noise_levels_sorted, uncorr_accs,
            marker=marker, markersize=4, linewidth=2, color=color_test_uc)
    ax.plot(ref_noise_levels_sorted, ref_accs,
            marker=None, linewidth=2, color='k', alpha=0.9, linestyle='--')
...
plt.savefig(os.path.join(figure_dirname, 'multi_mnist_{}.png'.format(use_metric)),
            bbox_extra_artists=[lgd] + extra_labels, bbox_inches='tight', pad_inches=0.02)
plt.savefig(os.path.join(figure_dirname, 'multi_mnist_{}.pdf'.format(use_metric)),
            bbox_extra_artists=[lgd] + extra_labels, bbox_inches='tight', pad_inches=0.02)
