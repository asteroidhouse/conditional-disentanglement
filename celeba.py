...
CELEBA_ATTRS = ['5_o_Clock_Shadow', 'Arched_Eyebrows', 'Attractive', 'Bags_Under_Eyes',
...
def get_celeba_raw_attrs():
    df = pandas.read_csv(os.path.join(CELEBA_ROOT, 'Anno', 'list_attr_celeba.txt'), skiprows=1, delimiter=r"\s+")
...
def get_celeba_dataset(filter_variable, target_variable1, target_variable2):
    ...
    # Load Split
    split_df = pandas.read_csv(os.path.join(
        CELEBA_ROOT, 'Eval', 'list_eval_partition.txt'), delimiter=r"\s+", names=['filename', 'partition'])

    for split_idx, split_name in zip([0, 1, 2], ['train', 'val', 'test']):
        for f_val in [0, 1]:
            idxs = df[(df[filter_variable] == f_val) & (split_df['partition'] == split_idx)].index.values
            Xs = X[idxs].float()
...
class CorrelatedCelebaDataset(torch.utils.data.Dataset):
    def __init__(self, data, labels, corr_matrix=None, noise=0.0, occlusion_patch_size=4):
        self.data = data
        self.labels = labels
        self.noise = noise
    ...
    def __getitem__(self, index):
        ...
        mask = data_utils.generate_occlusion_mask(self.noise, self.occlusion_patch_size, image_size=64)
...
def get_natural_correlated_celeba(factor1, factor2, splits=['train', 'val', 'test']):
    ...
    X = load_celeba_image_cache()
    # Load Split
    split_df = pandas.read_csv(os.path.join(CELEBA_ROOT, 'Eval', 'list_eval_partition.txt'), delimiter=r"\s+", names=['filename', 'partition'])
...
def get_correlated_celeba(factor1, factor2, train_corr, test_corr, splits=['train', 'val', 'test'], noise=0.0):
...
if __name__ == '__main__':
    ...
    if dataset_type == 'conditioned':
        ...
        loaders = dict([(k, torch.utils.data.DataLoader(datasets[k], batch_size=100,
        ...
        datasets = get_correlated_celeba(factor1=factor1,
                                         ...
                                         noise=noise)
        loaders = dict([(k, torch.utils.data.DataLoader(datasets[k], batch_size=100, shuffle=True if 'train' in k else False)) for k in datasets])
        # Plot the joint label distributions for the training and test sets to see how the filter variable affects them
