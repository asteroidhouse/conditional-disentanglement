...
* TODO: Add support for correlated dSprites
* TODO: Add support for disentanglement metrics (GBT feature matrix, unfairness score, etc.)
...
def plot_metrics():
    res_dir = defaultdict(list)
    for iteration in range(1, args.num_iterations//args.log_freq):
        with open(save_dir + '/metrics/metrics_' + str(iteration * args.log_freq) + '.pkl', 'rb') as f:
                data = pkl.load(f)
                for outer_key in data.keys():
                    if outer_key != 'fairness': #fairness has to many parameters...
                        if data[outer_key].keys() is not []:
                            for inner_key in data[outer_key].keys():
                                res_dir[outer_key + '\n' + inner_key].append(data[outer_key][inner_key])
                        else:
                            res_dir[outer_key].append(data[outer_key])
    ...
    for key in res_dir.keys():
        i += 1
        plt.subplot(4, 4, i)
        plt.plot(range(1, 1 + len(res_dir[key])), res_dir[key])
        plt.title(key, fontsize=10)
        plt.xlabel('Iteration', fontsize=10)
        if i >= 16:
            break
    plt.subplots_adjust(hspace=0.5, wspace=0.3) 
    plt.savefig(save_dir + '/metric_plots/metrics.png')
    plt.close(fig)
# --------------------------------------------------------
