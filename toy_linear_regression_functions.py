# Functions for toy_linear_regression_notebook.py
import numpy as np
...
def get_optimal_regression(cov_z_with_noise, A, W):
    ''' find optimal linear regression
    '''
...
def test_regression(cov_z_with_noise, A, W, R):
    ''' compute mse and variance explained for a given A, W, R
    '''
    ...
    substraction_matrix = np.array([[1, 0, -1, 0], [0, 1, 0, -1]])
    cov_diff = multi_dot([substraction_matrix, cov_s_and_s_hat, substraction_matrix.T])
...
def plot_noise_dependency(ax, correlation, loss_type, A, list_noise_level):
    ...
    results = np.zeros([
        len(list_noise_level),
        5]) #five values are for: ve_reference, ve, ve_test, ve_tests_min, ve_tests_max
    ...
    ax.fill_between(list_noise_level,
                        results[:, 3],
                        results[:, 4],
                        color=color_test, alpha=.1)
