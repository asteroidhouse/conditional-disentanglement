...
Example
-------
srun -p gpu --gres=gpu:1 --mem=16G python toy_linear_classification2.py 2 0
srun -p gpu --gres=gpu:1 --mem=16G python toy_linear_classification2.py 4 1
srun -p gpu --gres=gpu:1 --mem=16G python toy_linear_classification2.py 10 2
"""
import os
import sys
import csv
...
def sample_z_classification(num_samples, noise_level, correlation, dim):
    ...
    # elif correlation <0: # this gives the maximal possible anticorrelation (=-0.33 for dim=3)
...
class Net(nn.Module):
    ''' Discriminator used for disentanglement loss
    '''
...
class Discriminator():
    ...
    def eval_fakes(self, v):
        ...
        for d in range(self.dim):
            v_fake[d, :] = v_fake[d, torch.randperm(self.N)]
        #v_fake[1, :] = v_fake[1, torch.randperm(self.N)]
        prediction_fakes = self.net(v_fake).flatten()
...
def minimize(x, z, loss_terms, dim):
    ...
    '''
    # lr depends on number of attributes
    if dim == 2:
    ...
    for lr_discr in possible_lr_discr:
        ...
        if loss_terms == 'classification':
            ...
            discr_list = []
            for d in range(dim):
                discr_list.append(Discriminator_conditional(N, dim, 0.01, d, -1, z=z_discret))
                discr_list.append(Discriminator_conditional(N, dim, 0.01, d, 1, z=z_discret))
