...
def evaluate(dataloader):
  ...
  f1_acc_dict = {combo: num_correct_f1_dict[combo] / float(num_total_dict[combo])
                 for combo in num_total_dict.keys()}
  f2_acc_dict = {combo: num_correct_f2_dict[combo] / float(num_total_dict[combo])
                 for combo in num_total_dict.keys()}
...
base_dirs = [
    'saves/multi_mnist_cls',
    'saves/multi_mnist_cond',
    'saves/multi_mnist_uncond'
]
...
for base_dir in base_dirs:
  ...
  for subdir in subdirs:
    try:
      ...
      f1_classifier = torch.load(
          os.path.join(base_dir, subdir, '{}f1_classifier.pt'.format(ckpt_prefix))
      )
      f2_classifier = torch.load(
          os.path.join(base_dir, subdir, '{}f2_classifier.pt'.format(ckpt_prefix))
      )
      ...
      for test_corr in test_corr_list:
        classes = [3, 8]
        _, _, testset_correlated, c_to_i, i_to_c = data_utils.get_correlated_data(
            'mnist',
            classes=classes,
            train_corr=0.0,
            test_corr=test_corr,
