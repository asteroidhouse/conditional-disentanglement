...
Examples
--------
srun -p interactive --gres=gpu:1 --mem=16G python toy_evaluate_disentanglement.py
"""
import os
import sys
import ipdb
...
class DataSampler:
    ...
    def sample_observations_from_factors(self, z, random_state):
        # Should we add noise to z and then pass it through A?
        epsilon = random_state.randn(*z.shape) * np.sqrt(self.noise_level)
        z_and_noise = np.concatenate((z, epsilon), axis=1)  # (500, 4)
        x = np.dot(A, z_and_noise.T).T  # OR just z somehow ?
    ...
    def sample(self, num, random_state):
        ...
        # Take transposes to convert (2,100) --> (100,2) since the batch dimension needs to come first
        z = z.T
        x = x.T
...
exp_paths = [# ('cls', '/scratch/gobi1/pvicol/domain-adaptation/mi-branch-simplified/toy_cls_dim_2_noise_0.027825594022071243_corr_0_anticorr_0'),
             # ('uncond', ''),
             # ('cond', '')
             (2, '/scratch/gobi1/pvicol/domain-adaptation/mi-branch-simplified/toy_cls_dim_2_noise_1.6681005372000592_corr_0.95_anticorr_0'),
...
# for correlation in [0.0, 0.6, 0.95]:
for correlation in [0.0]:
    ...
    for dim, exp_dir in exp_paths:
        # dim = 4
        A = get_A(dim=dim)  # Putting this here so that A is available in global scope
        ...
        for fname in ['classification.pkl', 'classification_unconditional.pkl', 'classification_conditional.pkl']:
            ...
            # W = torch.eye(dim)
            # W = load_weights(os.path.join(exp_dir, 'classification.pkl'))
