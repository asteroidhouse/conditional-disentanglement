...
class DatasetMulti(torch.utils.data.Dataset):
  def __init__(self, data, label, dim, classes=None,
               c_to_i=None, i_to_c=None, transform=None,
               target_transform=None, corr=None, noise=0.0,
...
def get_correlated_data(dataset, classes, train_corr, test_corr, noise, occlusion_patch_size=4):
  ...
  trainset = Dataset(train_data, train_labels, classes=classes,
                     c_to_i=c_to_i, i_to_c=i_to_c, transform=train_transform,
                     corr_matrix=train_corr_matrix, noise=noise,
                     occlusion_patch_size=occlusion_patch_size)
  valset = Dataset(val_data, val_labels, classes=classes,
                   c_to_i=c_to_i, i_to_c=i_to_c, transform=train_transform,
                   corr_matrix=train_corr_matrix, noise=noise,
                   occlusion_patch_size=occlusion_patch_size)
  testset = Dataset(test_data, test_labels, classes=classes,
                    c_to_i=c_to_i, i_to_c=i_to_c, transform=test_transform,
                    corr_matrix=test_corr_matrix, noise=noise,
                    occlusion_patch_size=occlusion_patch_size)
...
def get_correlated_data_multi(dataset, classes, dim, train_corr, test_corr, noise, occlusion_patch_size=4):
  ...
  trainset = DatasetMulti(train_data, train_labels, dim=dim,
                          classes=classes, c_to_i=c_to_i, i_to_c=i_to_c,
                          transform=train_transform, corr=train_corr, noise=noise,
                          occlusion_patch_size=occlusion_patch_size)
  valset = DatasetMulti(val_data, val_labels, dim=dim,
                        classes=classes, c_to_i=c_to_i, i_to_c=i_to_c,
                        transform=train_transform, corr=train_corr, noise=noise,
                        occlusion_patch_size=occlusion_patch_size)
  testset = DatasetMulti(test_data, test_labels, dim=dim,
                         classes=classes,  c_to_i=c_to_i, i_to_c=i_to_c,
                         transform=test_transform, corr=0.0, noise=noise,
                         occlusion_patch_size=occlusion_patch_size)
